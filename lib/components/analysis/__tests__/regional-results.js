import {wrapMapComponent} from 'jest/component'
import {mockStores, mockRegionalAnalyses} from 'jest/mock-data'
...
test('RegionalResults snapshot(mount)', () => {
  const c = wrapMapComponent(RegionalResults, props)
  const t = c.mount()
...
test('RegionalResults with comparison snapshot(mount)', () => {
  ...
  const c = wrapMapComponent(RegionalResults, props, store)
  const t = c.mount()
  expect(t).toMatchSnapshot()
  t.unmount()
})
